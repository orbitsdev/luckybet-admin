...
<div x-data="{ sidebarOpen: false, profileOpen: false }" class="min-h-screen bg-[hsl(var(--background))]">
  <div class="flex min-h-screen">
    <!-- Sidebar -->
    <div :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-64 bg-[hsl(var(--card))] shadow-md transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 flex flex-col">
        <div class="flex items-center justify-between px-6 py-4">
            <span class="text-[hsl(var(--foreground))] font-extrabold text-2xl tracking-wide flex items-center gap-2">
                <!-- Lucky Bet Logo -->
            ...
            <button @click="sidebarOpen=false" class="text-[hsl(var(--foreground))] md:hidden focus:outline-none">
                <!-- X Icon -->
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
        </div>
        <nav class="flex-1 px-4 space-y-2 mt-4 bg-[hsl(var(--card))] border-r-2 border-[hsl(var(--border))]">
    <!-- Sidebar links with active highlight -->
    <a href="#" class="flex items-center px-3 py-2 rounded-lg bg-red-100 text-red-700 font-bold shadow-sm sidebar-link">
        <!-- Home Icon -->
        <svg class="w-6 h-6 mr-3 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7m-9 2v6m0 0h4m-4 0v6h4v-6m0 0V9m0 0l2 2m-2-2l-2 2" /></svg>
        Dashboard
    </a>
            <a href="#" class="flex items-center px-3 py-2 rounded-lg text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--primary))] transition-colors duration-200 font-medium sidebar-link">
                <!-- Table Icon -->
        ...
        <div class="mt-auto px-6 py-4">
            <button class="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-red-700 text-white font-bold shadow-lg hover:from-red-600 hover:to-red-800 transition-colors duration-200">
                <!-- Logout Icon -->
    ...
    <div class="flex-1 flex flex-col md:ml-64">
      <!-- Header -->
      <header class="flex items-center justify-between bg-[hsl(var(--card))] shadow border-b border-[hsl(var(--border))] px-6 py-4 sticky top-0 z-20">
        <div class="flex items-center gap-2">
          <button @click="sidebarOpen = !sidebarOpen" class="md:hidden text-gray-700 hover:bg-[hsl(var(--muted))] p-2 rounded-lg transition">
            <!-- Menu Icon -->
        ...
        <div class="flex items-center gap-4">
          <button class="relative hover:bg-[hsl(var(--muted))] transition">
            <!-- Bell Icon -->
          ...
          <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" class="flex items-center gap-2 focus:outline-none hover:bg-[hsl(var(--muted))] transition">
              <img src="https://i.pravatar.cc/40?img=3" alt="Avatar" class="w-9 h-9 rounded-full border-2 border-red-500 shadow" />
              <span class="hidden md:block font-semibold text-gray-700">Admin</span>
              <svg class="w-5 h-5 text-[hsl(var(--muted-foreground))]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
            </button>
            <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl py-2 z-50">
              <a href="#" class="flex items-center px-4 py-2 text-gray-700 hover:bg-[hsl(var(--muted))] transition">
                <!-- Cog Icon -->
              ...
              <a href="#" class="flex items-center px-4 py-2 text-gray-700 hover:bg-[hsl(var(--muted))] transition">
                <!-- Logout Icon -->
                <svg class="w-5 h-5 mr-2 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12h4" /></svg>
                Logout
              </a>
            </div>
          </div>
      ...
      <main class="flex-1 bg-gray-50 p-8 md:p-10 lg:p-12 space-y-8 min-h-screen">
        <!-- Cards Row -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Sales Card -->
                <div class="bg-white rounded-xl shadow p-6 flex items-center gap-4 border-t-4 border-red-500">
    <!-- Card: blue accent, flat, modern -->
                    <div class="flex-shrink-0 bg-gradient-to-r from-red-500 to-red-700 rounded-full p-3">
                        <!-- Currency Dollar Icon -->
                    ...
                    <div>
                        <div class="text-[hsl(var(--muted-foreground))] text-xs font-bold uppercase tracking-wide">Total Sales</div>
                        <div class="text-3xl font-extrabold text-[hsl(var(--foreground))]">₱1,250,000</div>
                    </div>
                </div>
                <!-- Winners Card -->
    ...
    <!-- Card: green accent, flat, modern -->
                    ...
                    <div>
                        <div class="text-[hsl(var(--muted-foreground))] text-xs font-bold uppercase tracking-wide">Winners</div>
                        <div class="text-3xl font-extrabold text-[hsl(var(--foreground))]">128</div>
                    </div>
                </div>
                <!-- Profit Card -->
                <div class="bg-white rounded-xl shadow p-6 flex items-center gap-4 border-t-4 border-red-400">
    <!-- Card: blue accent, flat, modern -->
                    <div class="flex-shrink-0 bg-red-400 rounded-full p-3">
                        <!-- Trending Up Icon -->
                    ...
                    <div>
                        <div class="text-[hsl(var(--muted-foreground))] text-xs font-bold uppercase tracking-wide">Profit</div>
                        <div class="text-3xl font-extrabold text-[hsl(var(--foreground))]">₱320,000</div>
                    </div>
                </div>
            </div>
            <!-- Fake Chart Row -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-xl shadow p-6 flex flex-col justify-between">
                    <div class="flex items-center justify-between mb-4">
                        <div class="text-[hsl(var(--foreground))] font-extrabold text-xl">Sales by Draw</div>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">Live</span>
                    </div>
                    <!-- Fake Bars -->
                    <div class="flex items-end h-32 gap-2">
                        <div class="w-1/6 bg-red-100 rounded-lg" style="height: 35%"></div>
                        <div class="w-1/6 bg-red-100 rounded-lg" style="height: 65%"></div>
                        <div class="w-1/6 bg-red-100 rounded-lg" style="height: 80%"></div>
                        <div class="w-1/6 bg-red-100 rounded-lg" style="height: 50%"></div>
                        <div class="w-1/6 bg-red-100 rounded-lg" style="height: 90%"></div>
                        <div class="w-1/6 bg-red-100 rounded-lg" style="height: 60%"></div>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow p-6 flex flex-col items-center justify-center">
                    ...
                    <div class="relative w-32 h-32 flex items-center justify-center">
                        ...
                        <svg viewBox="0 0 36 36" class="w-32 h-32">
                            <path class="text-red-500" stroke-width="3.8" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831" />
                        ...
                        <span class="absolute inset-0 flex items-center justify-center text-2xl font-bold text-red-600">68%</span>
                    </div>
                    <div class="flex gap-4 mt-4">
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">D4</span>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">3D</span>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-[hsl(var(--border))] text-gray-700">2D</span>
                    </div>
                </div>
            </div>
            <!-- Data Table -->
            <div class="bg-white rounded-xl shadow p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="text-[hsl(var(--foreground))] font-extrabold text-xl">Recent Draws</div>
                    <button class="px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-red-700 text-white font-bold shadow hover:from-red-600 hover:to-red-800 transition-colors duration-200">
                        View All
                    </button>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gradient-to-r from-red-500 to-red-700 text-white sticky top-0 z-10 rounded-t-xl">
                            <tr>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            <tr class="hover:bg-red-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap flex gap-2">
                                    <button class="p-2 rounded-full bg-red-100 text-red-700 hover:bg-red-200 transition-colors duration-200" title="View">
                                        <!-- Eye Icon -->
                            ...
                            <tr class="hover:bg-red-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-[hsl(var(--border))] text-gray-700 text-white  ">Cancelled</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap flex gap-2">
                                    <button class="p-2 rounded-full bg-red-100 text-red-700 hover:bg-red-200 transition-colors duration-200" title="View">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542 7z" /></svg>
